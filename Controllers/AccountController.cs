...
namespace CRM.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(UserManager<ApplicationUser> userManager, JwtService jwtService, IEmailSender emailSender)
        {
        ...
        public async Task<IActionResult> Login(LoginDto model)
        {
            ...
            if (model.RememberMe)
            {
                // If 'Remember Me' is checked, set the cookie to last for 30 days
                cookieOptions.Expires = DateTime.UtcNow.AddDays(30);
            }
            // Note: If model.RememberMe is false, we don't set an Expires date, 
            // making it a "Session Cookie" that expires when the browser closes.
